...
const activeClassName = 'inline-flex h-[32px] w-full items-center justify-center rounded-full bg-white px-2 text-[13px] font-medium text-[#7c3aed] shadow-[0_2px_6px_rgba(0,0,0,0.10)] sm:w-auto sm:min-w-[88px] sm:px-4';
const inactiveClassName = 'inline-flex h-[32px] w-full items-center justify-center rounded-full px-2 text-[13px] font-medium text-gray-500 transition hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7c3aed]/40 sm:w-auto sm:min-w-[88px] sm:px-4';
...
export function PeriodSelector({ value, from, to, onChange }: { value: SummaryPeriod; from?: string; to?: string; onChange: (value: SummaryPeriod, from?: string, to?: string) => void }) {
  ...
  return <div role="group" aria-label={t('merchant.revenue.period', 'Period')} className="grid w-full grid-cols-2 gap-1 rounded-full border border-gray-200 bg-gray-100 p-1 sm:inline-flex sm:w-auto sm:items-center">
