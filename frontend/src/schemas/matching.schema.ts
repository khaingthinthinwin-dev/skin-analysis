import { z } from 'zod'

export const matchQuerySchema = z.object({
  categoryId: z.string().optional().transform((val) => (val === '' ? undefined : val)),
  skinTypes: z.string().optional().transform((val) => (val === 'all' || val === '' ? undefined : val)),
  ingredients: z.string().optional(),
  minPrice: z.coerce.number().min(0).optional(),
  maxPrice: z.coerce.number().min(0).optional(),
  rating: z.coerce.number().min(1).max(5).optional(),
  // The sort bar defaults to "Newest" and the value is sent explicitly so the
  // option shown is always the order the API applies (BR-MATCH-025 allowlist).
  sort: z.enum(['matchScore', 'price', 'rating', 'createdAt']).optional().default('createdAt'),
  order: z.enum(['asc', 'desc']).optional().default('desc'),
  page: z.coerce.number().int().min(1).optional().default(1),
  limit: z.coerce.number().int().min(1).max(50).optional().default(12),
})

export type MatchQueryParams = z.infer<typeof matchQuerySchema>

export interface RecommendationProduct {
  id: string
  name: string
  slug: string
  brandName: string
  shortDescription: string | null
  price: string
  compareAtPrice: string | null
  avgRating: string
  reviewCount: number
  skinTypes: string[]
  images: string[]
  isInStock: boolean
  isFeatured: boolean
  matchScore: number | null
  categoryBadge: 'featured' | 'topRated' | 'bestSeller' | 'new' | null
}

export interface RecommendationResponse {
  data: RecommendationProduct[]
  meta: {
    page: number
    limit: number
    total: number
    totalPages: number
  }
  source: 'ai' | 'generic'
  analysisAge: number | null
  skinTypes: string[]
}

export interface AdSlide {
  adId: string
  title: string
  description: string | null
  imageUrl: string
  linkUrl: string | null
  ctaText: string
  priorityAmount: string | null
  shopName: string
}

export interface AdPanelResponse {
  data: AdSlide[]
  meta: {
    total: number
    maxAds: number
  }
}

export interface HistorySession {
  sessionId: string
  sessionDate: string
  skinTypesUsed: string[]
  products: HistoryRecommendation[]
}

export interface HistoryRecommendation {
  id: string
  name: string
  slug: string
  price: string
  images: string[]
  matchScore: number | null
}

export interface HistoryResponse {
  data: HistorySession[]
  meta: {
    page: number
    limit: number
    total: number
    totalPages: number
  }
}
