...
modular-reporter.ts writes test-results/{Module}/results.json
    ↓
update-pcl.cjs reads results + pcl-map.json
    ↓
...
Test title → PCL text mappings are stored in `e2e/pcl-map.json`.

To add a new mapping:

1. Add checklist item in the screen's `*_PCL.md`
2. Add mapping in `pcl-map.json`:
   ```json
   {
     "ModuleName": {
...
```
e2e/
├── pcl-map.json                  # Test title → PCL text mappings
