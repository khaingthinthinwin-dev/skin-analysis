...
The **Checkout & Purchase Module** (繝√ぉ繝・け繧｢繧ｦ繝医・雉ｼ蜈･繝｢繧ｸ繝･繝ｼ繝ｫ) is the buyer-facing purchase subsystem of the Cosmetics Finder platform. It covers the **Checkout Page** (`/checkout`) and the **Order Confirmation Page** (`/checkout/confirmation/:orderId`) 窶・converting cart contents into confirmed orders with status `placed`, validating stock availability, applying coupon discounts, calculating totals (subtotal 竏・discount), persisting order records for merchant fulfillment, and rendering the post-purchase confirmation (讖溯・險ｭ險域嶌 ﾂｧ1.1, ﾂｧ1.2, Requirement Spec ﾂｧ3.3, 逕ｻ髱｢鬆・岼險ｭ險域嶌 v1.0).

This module is **read-write**: it creates `orders`, `order_items`, `order_status_history`, and `inventory_transactions` records, decrements `products.stock_quantity`, increments `promotions.used_count`, and clears `cart_items` 窶・all within a single atomic database transaction (BR-CHECK-014). The module also renders the sponsored ad slide-down panel on the checkout page (UC-CHECK-008, BR-AD-001~012).

**Order History, Order Detail, and Order Tracking** (UC-CHECK-005, UC-CHECK-006, UC-CHECK-007 per 讖溯・險ｭ險域嶌 ﾂｧ2.1/ﾂｧ2.3) are explicitly **out of Detailed Design scope for this module** 窶・they are implemented by the **Order Insights module** (逕ｻ髱｢鬆・岼險ｭ險域嶌 v1.0 revision history: "Order History, Order Detail, and Order Tracking pages are developed by the order status screen team"). This module creates the orders that Order Insights later reads; the order-status state machine itself belongs to Order Insights' DD_01 ﾂｧ3.
...
| UC-CHECK-005 | View Order History | **Out of Detailed Design scope for this module 窶・implemented by the Order Insights module.** The FDS ﾂｧ2.1/ﾂｧ2.3 includes this use case for workflow continuity (BR-HIST-001~003), but the screen itself (`/orders`), its API (`GET /api/v1/orders`), and its field-level design belong to Order Insights' DD_02 and DD_03. This module creates the orders that Order Insights lists here. |
| UC-CHECK-006 | View Order Detail | **Out of Detailed Design scope for this module 窶・implemented by the Order Insights module.** The FDS ﾂｧ2.1/ﾂｧ2.3 includes this use case for workflow continuity, but the screen (`/orders/:orderId`), its API (`GET /api/v1/orders/:id`), and its field-level design belong to Order Insights' DD_02 and DD_03. This module creates the orders whose detail Order Insights renders. |
| UC-CHECK-007 | Track Order | **Out of Detailed Design scope for this module 窶・implemented by the Order Insights module.** The FDS ﾂｧ2.1/ﾂｧ2.3 includes this use case for workflow continuity, but the screen (`/orders/:orderId/tracking`), its API (`GET /api/v1/orders/:id/tracking`), the order-status state machine, and its field-level design belong to Order Insights' DD_01 ﾂｧ3, DD_02, and DD_03. This module creates the orders whose tracking timeline Order Insights renders. |
...
## 3. Business Workflow 窶・Checkout to Confirmation
...
flowchart TD
    ...
    I --> J[GET /api/v1/ads?placement=checkout_top<br/>Load Sponsored Ad Slot 窶・parallel, non-blocking]
    J --> K[Checkout Page /checkout<br/>1. Review Order Summary<br/>2. Enter Shipping Address<br/>3. Select Payment Method<br/>4. Apply Coupon 窶・optional<br/>5. Review Total]
    K --> L{Apply Coupon?}
    ...
    U --> K
    T -- Yes --> V[POST /api/v1/orders<br/>Create Order + Items + Status History<br/>+ Inventory Transactions<br/>Decrement Stock, Clear Cart<br/>窶・Atomic Transaction]
    V --> W{Success?}
    ...
    Z -.->|OUT OF SCOPE| AA[Order History /orders<br/>窶・Order Insights Module]
    AA -.->|OUT OF SCOPE| AB[Order Detail /orders/:orderId<br/>窶・Order Insights Module]
    AB -.->|OUT OF SCOPE| AC[Order Tracking /orders/:orderId/tracking<br/>窶・Order Insights Module]
...
**Scope boundary:** The solid-box nodes above are designed in this module (DD_CHECK-02 through DD_CHECK-06). The dashed-box nodes are designed by the Order Insights module (DD_OI-01 through DD_OI-06).
...
|--------|---------------|
| **Authentication** | JWT Bearer Token passed via `Authorization` header for all checkout and order-placement endpoints (BR-CHECK-001, 讖溯・險ｭ險域嶌 ﾂｧ10.1). Refresh token stored in httpOnly cookie for session management. |
| **Guest Checkout Blocking** | Checkout is unavailable to guests. When a guest user attempts to access `/checkout`, an alert modal is displayed: "Please log in to complete your purchase." Clicking [Log in] navigates to the login page (`/login`) 窶・讖溯・險ｭ險域嶌 ﾂｧ1.3, 逕ｻ髱｢鬆・岼險ｭ險域嶌 ﾂｧ2.2. |
| **Role-Based Access** | Checkout and order placement are restricted to the `buyer` role (BR-CHECK-003). Merchants and Admins receive `403 Forbidden` with message "Shopping features are only available to buyers" (讖溯・險ｭ險域嶌 ﾂｧ10.3). Enforced by backend `requireBuyerRole` guard / `RolesGuard`. |
| **Ownership Rules** | Buyers can only place orders for themselves; `buyer_id` is derived from the JWT, never from request body (讖溯・險ｭ險域嶌 ﾂｧ10.4). Coupons are merchant-created, buyer-applied 窶・validated by backend. |
| **Ad Slot Exception** | `GET /api/v1/ads?placement=checkout_top` is publicly cacheable (no auth header required), but the page-level authentication gate still applies (讖溯・險ｭ險域嶌 ﾂｧ10.2). |
---
...
| Layer | Files |
|-------|-------|
| **Backend Controllers** | `checkout.controller.ts` (routes under `/api/v1/checkout`), `orders.controller.ts` (routes under `/api/v1/orders` 窶・order placement only) |
...
|--------|----------|-------------|:-------------:|
| `GET` | `/api/v1/checkout` | Load checkout page data 窶・cart items, subtotal, stock status, user addresses | Yes (Buyer) |
...
| `POST` | `/api/v1/orders` | Place order 窶・create order, order items, status history, inventory transactions; decrement stock; clear cart; return order confirmation | Yes (Buyer) |
...
This module is **read-write** 窶・it writes to these tables during order placement (unlike Order Insights, which is fully read-only).
...
| i18next | Internationalization (EN / MY / JA) 窶・checkout labels, error messages, confirmation text | `frontend/src/i18n.ts` |
...
|------------------|---------|
| [DD_CHECK-02](./DD_Checkout_Purchase_02_FRONTEND_Page.md) | Frontend page design 窶・Checkout and Order Confirmation screens |
| [DD_CHECK-03](./DD_Checkout_Purchase_03_API_ENDPOINTS.md) | Backend REST API contract 窶・`/checkout`, `/orders`, `/ads` endpoints |
| [DD_CHECK-04](./DD_Checkout_Purchase_04_DTOS_AND_TYPES.md) | DTO and type definitions 窶・shipping address, place-order, coupon, ad slot |
| [DD_CHECK-05](./DD_Checkout_Purchase_05_BUSINESS_LOGIC.md) | Backend business rules 窶・order placement transaction, coupon validation, stock decrement |
| [DD_CHECK-06](./DD_Checkout_Purchase_06_TEST_SPEC.md) | Test specification 窶・checkout flow, coupon application, order placement, ad slot |
| [讖溯・險ｭ險域嶌_Checkout_Purchase](../讖溯・險ｭ險域嶌_Checkout_Purchase.md) | Full functional specification (use cases, workflows, business rules, error handling) |
| [逕ｻ髱｢鬆・岼險ｭ險域嶌_Checkout_Purchase](../逕ｻ髱｢鬆・岼險ｭ險域嶌_Checkout_Purchase.md) | Screen items specification 窶・Checkout + Order Confirmation fields and layout |
| [DD_OI-01](../Order_Insights/Detailed%20Design/DD_Order_Insights_01_MODULE_OVERVIEW.md) | Order Insights module overview 窶・owns the order-status state machine and Order History/Detail/Tracking screens that this module's orders flow into |
