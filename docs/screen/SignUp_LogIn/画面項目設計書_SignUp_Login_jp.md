# 画面項目設計書 — サインアップ / ログイン / パスワードリセット

**ドキュメントID:** SKM-SIS-SCR-001  
**対象画面:** 認証（サインアップ / ログイン / パスワードリセット）  
**サブシステム:** ユーザー認証  
**機能ID:** FN-AUTH-001  
**バージョン:** 4.0  
**作成日:** 2026-08-04  
...
| 4.0 | 2026-08-20 | シニアシステムエンジニア | パスワード忘れた（`/forgot-password`）およびパスワードリセット（`/reset-password`）画面を追加。新しい項目定義、イベント仕様、バリデーションエラーマッピング、APIレスポンスマッピング、i18nキー、テストケースを追加。 |
...
### 2.1 目的
サインアップ、ログイン、パスワードリセットページは、Cosmetics Finderプラットフォームにおけるユーザー認証のエントリーポイントです。新規ユーザーがアカウント（購入者または出品者）を作成し、既存ユーザーがメールアドレスとパスワードで認証し、セッション管理用のJWTトークンを受け取ることができます。さらに、すべてのユーザーが安全なメールリンクを通じて忘れたパスワードを回復することができます。

### 2.2 対象ユーザーと権限

| 属性 | 値 |
| :--- | :--- |
| **プライマリアクター** | 未認証ビジター（サインアップ、パスワードリセットリクエスト）、認証済みユーザー（ログイン）、リセットトークンを持つ任意のユーザー（パスワードリセット） |
| **必要認証** | なし（サインアップ、パスワード忘れた）、JWTベアラートークン（ログイン/セッション）、リセットトークン（パスワードリセット） |
| **データスコープ** | 新規ユーザー作成、既存ユーザー資格情報検証、パスワード回復 |
| **アクセス制御** | 公開ルート — ガード不適用 |
...
8. **パスワード回復** — メールでパスワードリセットリンクをリクエストし、トークン検証付きで新しいパスワードを設定。
...
| 9 | `lnkForgotPassword` | パスワード忘れたリンク | リンク（`<Link>`） | 文字列 | — | テキスト: "パスワードをお忘れですか？" | — | — | `/forgot-password`にナビゲート。パスワードフィールドの下に右寄せ。 |
| 10 | `btnLogin` | ログインボタン | ボタン（`submit`、`default`） | — | — | 表示中。テキスト: "ログイン" | — | — | 全幅。ローディング: スピナー + "ログイン中..."。ローディング中は無効。 |
| 11 | `lblNoAccount` | 新規登録プロンプト | 静的ラベル | 文字列 | — | テキスト: "アカウントをお持ちでないですか？" | — | — | フッターテキスト。 |
| 12 | `lnkSignUp` | 新規登録リンク | リンク（`<Link>`） | 文字列 | — | テキスト: "作成する" | — | — | `/register`にナビゲート。 |

### 4.4 セクション [E]: ログインフッターコントロール

| No. | Item ID | 項目名（論理名） | コンポーネントタイプ | データ型＆最大長 | 必須 | 初期状態／デフォルト値 | 入力制約／フォーマット | データソース／DBマッピング | 備考／ビジネスルール |
| :---: | :--- | :--- | :--- | :--- | :---: | :--- | :--- | :--- | :--- |
| 13 | `btnLanguageToggle` | 言語切替 | トグルグループ | 列挙型 | — | デフォルト: ブラウザ言語または"en" | 選択肢: EN, JA, MY | — | すべてのi18nキーを切替。localStorageに永続化。 |
| 14 | `btnThemeToggle` | テーマ切替 | アイコンボタン | 列挙型 | — | デフォルト: システム設定 | 選択肢: light, dark, system | — | light → dark → systemで切替。`next-themes`を使用。 |
...
### 4.6 セクション [I]: パスワード忘れたフォーム

| No. | Item ID | 項目名（論理名） | コンポーネントタイプ | データ型＆最大長 | 必須 | 初期状態／デフォルト値 | 入力制約／フォーマット | データソース／DBマッピング | 備考／ビジネスルール |
| :---: | :--- | :--- | :--- | :--- | :---: | :--- | :--- | :--- | :--- |
| 37 | `lblForgotTitle` | パスワード忘れたタイトル | 静的ラベル（`<h2>`） | 文字列 | — | 表示中。テキスト: "パスワードをお忘れですか？" | — | ハードコードUIテキスト | Tailwind: `text-2xl font-bold text-center`。 |
| 38 | `lblForgotDesc` | パスワード忘れた説明 | 静的ラベル（`<p>`） | 文字列 | — | 表示中。テキスト: "メールアドレスを入力すると、リセットリンクを送信します。" | — | ハードコードUIテキスト | Tailwind: `text-muted-foreground text-center`。 |
| 39 | `lblForgotEmail` | メールアドレスラベル | 静的ラベル（`<label>`） | 文字列 | — | 常に表示。テキスト: "メールアドレス" | — | ハードコードUIテキスト | `htmlFor`/`id`で`txtForgotEmail`に関連付け。 |
| 40 | `txtForgotEmail` | メールアドレス入力 | 入力（`email`） | 文字列(255) | 必須 | 空。プレースホルダー: "user@example.com" | フォーマット: 有効なメールアドレス。最大長: 255。 | — | AutoFocus: true。AutoComplete: `email`。InputMode: `email`。 |
| 41 | `btnSendResetLink` | リセットリンク送信ボタン | ボタン（`submit`、`default`） | — | — | 表示中。テキスト: "リセットリンクを送信" | — | — | 全幅。ローディング: スピナー + "送信中..."。ローディング中またはフォーム無効時は無効。 |
| 42 | `lnkBackToLoginForgot` | ログインに戻るリンク | リンク（`<Link>`） | 文字列 | — | テキスト: "ログインに戻る" | — | — | `/login`にナビゲート。 |

### 4.7 セクション [J]: パスワードリセットフォーム

| No. | Item ID | 項目名（論理名） | コンポーネントタイプ | データ型＆最大長 | 必須 | 初期状態／デフォルト値 | 入力制約／フォーマット | データソース／DBマッピング | 備考／ビジネスルール |
| :---: | :--- | :--- | :--- | :--- | :---: | :--- | :--- | :--- | :--- |
| 43 | `lblResetTitle` | パスワードリセットタイトル | 静的ラベル（`<h2>`） | 文字列 | — | 表示中。テキスト: "パスワードをリセット" | — | ハードコードUIテキスト | Tailwind: `text-2xl font-bold text-center`。 |
| 44 | `lblResetDesc` | パスワードリセット説明 | 静的ラベル（`<p>`） | 文字列 | — | 表示中。テキスト: "新しいパスワードを入力してください。" | — | ハードコードUIテキスト | Tailwind: `text-muted-foreground text-center`。 |
| 45 | `lblNewPassword` | 新しいパスワードラベル | 静的ラベル（`<label>`） | 文字列 | — | 常に表示。テキスト: "新しいパスワード" | — | ハードコードUIテキスト | `htmlFor`/`id`で`txtNewPassword`に関連付け。 |
| 46 | `txtNewPassword` | 新しいパスワード入力 | 入力（`password`） | 文字列(128) | 必須 | 空。プレースホルダー: "新しいパスワードを作成" | 複雑な正規表現（§4.5.1参照）。 | — | AutoFocus: true。AutoComplete: `new-password`。表示/非表示切替可能。 |
| 47 | `btnShowNewPassword` | パスワード表示/非表示 | アイコンボタン | — | — | 表示中。アイコン。 | — | — | `txtNewPassword`のタイプを`password`と`text`で切替。 |
| 48 | `lstResetPasswordRequirements` | パスワード要件 | ヘルパーテキストリスト | — | — | パスワードフィールドの下に表示中 | 5つの要件のチェックリスト | — | ユーザー入力時にリアルタイムフィードバック。適合時に緑のチェック。登録と同じルール。 |
| 49 | `lblResetConfirmPassword` | パスワード確認ラベル | 静的ラベル（`<label>`） | 文字列 | — | 常に表示。テキスト: "パスワード確認" | — | ハードコードUIテキスト | `htmlFor`/`id`で`txtResetConfirmPassword`に関連付け。 |
| 50 | `txtResetConfirmPassword` | パスワード確認入力 | 入力（`password`） | 文字列(128) | 必須 | 空。プレースホルダー: "新しいパスワードを再入力" | `txtNewPassword`と一致すること。 | — | AutoComplete: `new-password`。表示/非表示切替可能。 |
| 51 | `btnShowResetConfirmPassword` | パスワード確認表示/非表示 | アイコンボタン | — | — | 表示中。アイコン。 | — | — | `txtResetConfirmPassword`のタイプを切替。 |
| 52 | `btnResetPassword` | パスワードリセットボタン | ボタン（`submit`、`default`） | — | — | 表示中。テキスト: "パスワードリセット" | — | — | 全幅。ローディング: スピナー + "リセット中..."。ローディング中またはフォーム無効時は無効。 |
| 53 | `lnkBackToLoginReset` | ログインに戻るリンク | リンク（`<Link>`） | 文字列 | — | テキスト: "ログインに戻る" | — | — | `/login`にナビゲート。 |
...
### 5.6 ナビゲーションリンク（`lnkSignUp` / `lnkSignIn` / `lnkForgotPassword` / `lnkBackToLoginForgot` / `lnkBackToLoginReset` onClick）
- **トリガー:** ユーザーが新規登録、ログイン、パスワード忘れた、またはログインに戻るリンクをクリック。
- **処理ロジック:**
  1. React Routerで`/register`、`/login`、または`/forgot-password`にナビゲート。
...
### 5.10 パスワード忘れたフォーム送信（`btnSendResetLink` onClick）
- **トリガー:** ユーザーが「リセットリンクを送信」ボタンをクリック。
- **処理ロジック:**
  1. **クライアント側事前チェック:** フォームがバリデーションを受ける — メールアドレス形式が有効。
  2. **バックエンド送信:** `POST /api/v1/auth/forgot-password` に `{ email }` を送信。
  3. **バックエンド実行:** メールアドレス形式を検証。レート制限を確認（メールあたり1時間に最大3回）。メールアドレスでユーザーを検索。ユーザーが存在する場合: 未使用の以前のトークンを無効化、セキュアトークンを生成、24時間の有効期限で`password_reset_tokens`テーブルにハッシュ化して保存、リセットメールを送信。メールアドレスの有無に関わらず常に同じレスポンスを返す。
  4. **実行後UI:** フォームを成功メッセージに置き換え。「ログインに戻る」リンクを表示。
- **例外処理:**
  - `AUTH_008`（429）: リトライ秒数付きレート制限メッセージを表示。
  - `VAL-AUTH-040`（400）: 無効なメールアドレスのフィールドレベルインラインエラーを表示。
  - `SYS_001`（500）: 「問題が発生しました。もう一度お試しください」を表示。
  - ネットワークエラー: 「ネットワークエラー。接続を確認してください」を表示。

### 5.11 パスワードリセットフォーム送信（`btnResetPassword` onClick）
- **トリガー:** ユーザーが「パスワードリセット」ボタンをクリック（メールリンクをクリックした後）。
- **処理ロジック:**
  1. **クライアント側事前チェック:** 厳格なバリデーション — パスワードが強度要件を満たす、パスワード一致、URLにトークン存在。
  2. **バックエンド送信:** `POST /api/v1/auth/reset-password` に `{ token, password }` を送信。
  3. **バックエンド実行:** トークン形式を検証。受信トークンをハッシュ化。`token_hash`でトークンレコードを検索。検証: トークン存在、未使用、有効期限切れでない（24時間）。トークンの`user_id`でユーザーを検索。パスワード強度を検証。新しいパスワードをArgon2でハッシュ化。ユーザーのパスワードを更新。トークンを`used = TRUE`に設定。このユーザーの他のすべての未使用トークンを無効化。
  4. **実行後UI:** 成功メッセージを表示: "パスワードが正常にリセットされました。" 「ログインに戻る」リンクを表示。
- **例外処理:**
  - `AUTH_009`（400）: `alertError`に「無効または期限切れのリセットリンクです。新しいリセットをリクエストしてください。」を表示。
  - `VAL-AUTH-041`（400）: パスワード強度エラーをインライン表示。
  - `VAL-AUTH-042`（400）: 「パスワードが一致しません」をインライン表示。
  - `SYS_001`（500）: 「問題が発生しました。もう一度お試しください」を表示。
  - ネットワークエラー: 「ネットワークエラー。接続を確認してください」を表示。

### 5.12 パスワードリセットページ読み込み時のトークン検証
- **トリガー:** ユーザーが`/reset-password?token=xxx`にナビゲート。
- **処理ロジック:**
  1. URLクエリパラメータから`token`を抽出。
  2. トークンが存在しない場合、エラーを表示し`/login`にリダイレクト。
  3. トークンは送信時に検証（バックエンドが有効期限、使用済みを確認）。
  4. トークンが無効/期限切れの場合、エラーを表示し新しいリセットをリクエストするリンクを提供。
- **例外処理:**
  - トークンなし: `/login`にリダイレクト。
  - 無効/期限切れトークン（送信時）: エラー表示、`/forgot-password`へのリンク。
...
### 6.3 パスワード忘れたバリデーションエラー

| エラーコード | 対象フィールド | 条件／評価ロジック | UI/UX表示スタイル | デフォルトエラーメッセージ（EN） | デフォルトエラーメッセージ（JA） |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **VAL-AUTH-040** | `txtForgotEmail` | メールアドレスが空または無効な形式 | 赤いボーダー。フィールド下のテキスト。 | "Email is required" / "Invalid email address" | "メールアドレスは必須です" / "メールアドレスが無効です" |
| **VAL-AUTH-041** | `txtForgotEmail` | メールアドレスが255文字を超える | 赤いボーダー。フィールド下のテキスト。 | "Email must not exceed 255 characters" | "メールアドレスは255文字以内にしてください" |
| **AUTH_008** | `alertError` | レート制限（429レスポンス） | アラートバナー（破壊的） | "Too many attempts. Please wait {seconds} seconds" | "試行回数が多すぎます。{seconds}秒お待ちください" |
| **SYS_001** | `alertError` | サーバーエラー（500レスポンス） | アラートバナー（破壊的） | "Something went wrong. Please try again" | "問題が発生しました。もう一度お試しください" |
| **NET_ERR** | `alertError` | ネットワークエラー | アラートバナー（破壊的） | "Network error. Please check your connection" | "ネットワークエラー。接続を確認してください" |

### 6.4 パスワードリセットバリデーションエラー

| エラーコード | 対象フィールド | 条件／評価ロジック | UI/UX表示スタイル | デフォルトエラーメッセージ（EN） | デフォルトエラーメッセージ（JA） |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **VAL-AUTH-042** | `txtNewPassword` | パスワードが空 | 赤いボーダー。フィールド下のテキスト。 | "Password is required" | "パスワードは必須です" |
| **VAL-AUTH-043** | `txtNewPassword` | パスワード < 8文字 | 赤いボーダー。フィールド下のテキスト。 | "Password must be at least 8 characters" | "パスワードは8文字以上である必要があります" |
| **VAL-AUTH-044** | `txtNewPassword` | パスワード > 128文字 | 赤いボーダー。フィールド下のテキスト。 | "Password must not exceed 128 characters" | "パスワードは128文字以内にしてください" |
| **VAL-AUTH-045** | `txtNewPassword` | 大文字が不足 | 赤いボーダー。フィールド下のテキスト。 | "Password must contain at least one uppercase letter" | "パスワードには大文字を含めてください" |
| **VAL-AUTH-046** | `txtNewPassword` | 小文字が不足 | 赤いボーダー。フィールド下のテキスト。 | "Password must contain at least one lowercase letter" | "パスワードには小文字を含めてください" |
| **VAL-AUTH-047** | `txtNewPassword` | 数字が不足 | 赤いボーダー。フィールド下のテキスト。 | "Password must contain at least one number" | "パスワードには数字を含めてください" |
| **VAL-AUTH-048** | `txtNewPassword` | 特殊文字が不足 | 赤いボーダー。フィールド下のテキスト。 | "Password must contain at least one special character (@$!%*?&)" | "パスワードには特殊文字を含めてください" |
| **VAL-AUTH-049** | `txtResetConfirmPassword` | パスワードが一致しない | 赤いボーダー。フィールド下のテキスト。 | "Passwords do not match" | "パスワードが一致しません" |
| **AUTH_009** | `alertError` | 無効または期限切れリセットトークン（400レスポンス） | アラートバナー（破壊的） | "Invalid or expired reset link. Please request a new one." | "無効または期限切れのリセットリンクです。新しいリセットをリクエストしてください。" |
| **AUTH_008** | `alertError` | レート制限（429レスポンス） | アラートバナー（破壊的） | "Too many attempts. Please wait {seconds} seconds" | "試行回数が多すぎます。{seconds}秒お待ちください" |
| **SYS_001** | `alertError` | サーバーエラー（500レスポンス） | アラートバナー（破壊的） | "Something went wrong. Please try again" | "問題が発生しました。もう一度お試しください" |
| **NET_ERR** | `alertError` | ネットワークエラー | アラートバナー（破壊的） | "Network error. Please check your connection" | "ネットワークエラー。接続を確認してください" |
...
**注意:** このレスポンスはメールアドレスの有無に関わらず返されます（メールアドレス列挙防止のため）。

### 8.6 パスワード忘れたエラーレスポンス（レート制限）
...
- [ ] パスワード最小長（8）が適用される
- [ ] パスワード表示/非表示切替が機能する
- [ ] パスワード忘れたリンクが`/forgot-password`にナビゲートする
- [ ] 有効なデータでフォームが送信される
...
### 12.4 パスワード忘れたフォームテスト

- [ ] メールアドレスバリデーションが有効な形式を受け入れる
- [ ] メールアドレスバリデーションが無効な形式を拒否する
- [ ] メールアドレス最大長（255）が適用される
- [ ] 有効なメールアドレスでフォームが送信される
- [ ] 送信後に成功メッセージが表示される
- [ ] フォームが成功メッセージに置き換えられる（メールアドレス入力が表示されない）
- [ ] 「ログインに戻る」リンクが`/login`にナビゲートする
- [ ] 試行回数过多でエラーアラートが表示される（AUTH_008）
- [ ] 送信中にローディング状態が表示される
- [ ] メールアドレス入力に自動フォーカスされる
- [ ] 言語切替がすべてのラベルを切り替える
- [ ] テーマ切替が機能する
- [ ] キーボードナビゲーションが機能する（Tab、Enter）
- [ ] ネットワークエラーが適切なメッセージを表示する

### 12.5 パスワードリセットフォームテスト
...
- [ ] リセット後に成功メッセージが表示される
- [ ] 「ログインに戻る」リンクが`/login`にナビゲートする
- [ ] 無効/期限切れトークンでエラーアラートが表示される（AUTH_009）
- [ ] 送信中にローディング状態が表示される
- [ ] 新しいパスワード入力に自動フォーカスされる
- [ ] 両方のパスワードフィールドで表示/非表示切替が機能する
- [ ] 言語切替がすべてのラベルを切り替える
- [ ] テーマ切替が機能する
- [ ] キーボードナビゲーションが機能する（Tab、Enter）
- [ ] ネットワークエラーが適切なメッセージを表示する
- [ ] URLにトークンがない場合`/login`にリダイレクトする
