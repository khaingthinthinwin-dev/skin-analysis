...
- 評価: Lucide `Star`アイコン、ハーフスター対応、レビュー数リンク。
- 価格: 現在の価格は太字、比較価格は取り消し線、通貨フォーマット。
- タグ: 肌タイプの小さなピルバッジ（例: "Oily"、"Sensitive"）。
- お気に入り: ハートアイコントグル、クリック時アニメーション。
...
**ダッシュボードルール:**
- KPIカード: 大きな数字、ラベル、トレンドインジケーター（パーセンテージ付き上下矢印）。
- チャート: Rechartsまたは同等のものを使用。レスポンシブ、インタラクティブ。
- テーブル: ソート可能なカラム、ページネーション、ステータスバッジ。
...
**管理者ダッシュボードルール:**
- 保留アクションのアラートバッジ（出品者承認、レビューモデレーション）。
- トレンドインジケーター付きのプラットフォーム全体の指標。
- 一般的な管理者タスクのクイックアクションボタン。
- すべてのチャートはインタラクティブであること（ホバーツールチップ、クリックでドリルダウン）。
...
|-------|-----------|-----------|-------|
| `--background` | 白 | ダークネイビー | ページ背景 |
| `--foreground` | ニアブラック | ニアホワイト | プライマリテキスト |
| `--primary` | ブランドブルー | ブランドブルー | プライマリアクション、リンク |
| `--primary-foreground` | 白 | 白 | プライマリ上のテキスト |
| `--secondary` | ライトグレー | ダークグレー | セカンダリアクション |
| `--muted` | 非常に明るいグレー | 非常に暗いグレー | ミュートされた背景 |
| `--muted-foreground` | グレー | ライトグレー | セカンダリテキスト |
| `--destructive` | 赤 | 赤 | 削除、エラーアクション |
| `--border` | ライトグレー | ダークグレー | ボーダー |
| `--ring` | ブランドブルー | ブランドブルー | フォーカスリング |
...
text-foreground   → プライマリテキスト
text-muted-foreground → セカンダリテキスト
border-border     → すべてのボーダー
```
...
- ソート可能なカラム: ヘッダーをクリックしてソート、矢印インジケーターを表示。
- 選択可能な行: 最初のカラムのチェックボックス。
- ページネーション: テーブルの下部、現在の範囲を表示。
...
- タイトルは記述的で簡潔であること。
- アクション: 左にキャンセル（アウトライン）、右にプライマリアクション。
...
| 保留中 | 黄色（`bg-yellow-100 text-yellow-800`） | Pending |
| 処理中 | 青（`bg-blue-100 text-blue-800`） | Processing |
...
| 低在庫 | 琥珀（`bg-amber-100 text-amber-800`） | Low Stock |
...
- カラーはセマンティックな意味を伝えること（緑=良好、赤=悪い、黄=警告）。
